...
export class PersistentDatabase {
    ...
    async saveDocument(doc) {
        if (!doc.url)
            throw new Error('Document must have URL');
    ...
    async saveQuery(query) {
        if (!query.id)
            throw new Error('Query must have ID');
        await this.runQuery(`
    ...
    async saveAuthority(authorityData) {
        const { name, official_name, jurisdiction, jurisdiction_level, website, contact_phone, contact_email, contact_chatbot, contact_hours, postal_address } = authorityData;
        await this.runQuery(`
