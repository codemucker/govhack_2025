...
import { PersistentDatabase, Document } from './persistent-database.js';
...
interface SeedDocumentList {
  commonwealth: SeedDocument[];
  nsw: SeedDocument[];
  qld: SeedDocument[];
  vic: SeedDocument[];
  wa: SeedDocument[];
...
export class DocumentSeeder {
  ...
  private seedDocuments: SeedDocumentList;
  private austliiScraper: AustLIIScraper;
  private intelligentDiscovery: IntelligentDocumentDiscovery;

  constructor(database: PersistentDatabase) {
    this.db = database;
  ...
  getSeedDocumentList(): SeedDocumentList {
    return {
  ...
  async seedAllDocuments(): Promise<void> {
  ...
  async createSyntheticDocument(docInfo: SeedDocument): Promise<string> {
...
`;
  ...
  async getSeadingStats(): Promise<any> {
    ...
    return {
      ...
      by_jurisdiction: Object.keys(this.seedDocuments).reduce((acc, jurisdiction) => {
        acc[jurisdiction] = this.seedDocuments[jurisdiction as keyof SeedDocumentList].length;
        return acc;
